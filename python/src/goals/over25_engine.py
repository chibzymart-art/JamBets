...
class Over25GoalsEngine:
    ...
    def run(self, max_fixtures: int = 500, wipe_pending: bool = False) -> Dict[str, Any]:
        ...
        for f in fixtures:
            ...
            # =================================================================
            # QUALITY GATE 5: Raised Value Thresholds
            # Over 2.5: Requires >= 68% | 1H Over 0.5: Requires >= 76%
            # =================================================================
            qualifies_over25 = final_p_over25 >= 0.68 and intent.is_favorable_for_over
            qualifies_ht05 = final_p_ht05 >= 0.76 and intent.match_intent_index >= 0.95

            # --- MARKET 1: Over 2.5 Goals ---
            if qualifies_over25:
                if (fid, "over_2.5_goals") not in locked_keys:
                    ...
                    records_to_upsert.append({
                        ...
                        "avg_first_goal_minute": max(15, min(36, int(35 - (final_p_ht05 * 18)))),
            ...
            # --- MARKET 2: 1H Over 0.5 Goals ---
            if qualifies_ht05:
                if (fid, "ht_over_0.5_goals") not in locked_keys:
                    tier_ht05 = "EARLY_STRIKE" if final_p_ht05 >= 0.82 else "TEMPO_HIGH"
                    records_to_upsert.append({
                        "fixture_id": fid,
        ...
        print(f"✅ [Over25Engine] Completed run. Published {len(records_to_upsert)} predictions "
              f"({over25_published} Over 2.5 Goals, {ht05_published} 1H Over 0.5 Goals).")
